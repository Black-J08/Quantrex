"""Single instrument execution mode for Quantrex Backtest."""
...
class SingleInstrumentExecution(ExecutionMode):
    """Single-instrument sequential execution logic."""
    ...
    def execute(
        ...
        # Set up logging for this symbol
        self._run_logger.ensure_run_log_file(staging_dir, [symbol])

        # Create context for single instrument
        origin_time = spec.adapter.get_origin_time()
        ...
        base_data = raw_data[symbol][base_timeframe]
        if not base_data:
        ...
        symbol_tf_to_rows = self._build_row_lookup(
            symbols=[symbol],
            raw_data=raw_data,
            required_timeframes=list(raw_data[symbol].keys()),
        ...
        for idx, base_row in enumerate(base_data):
            try:
                ...
                if isinstance(timestamp, str):
                    ts = datetime.strptime(timestamp, spec.adapter.datetime_format)
                else:
                    ts = timestamp
        ...
        if final_run_dir != staging_dir:
            run_dir = self._directory_manager.promote_staging_to_final(
                staging_dir, final_run_dir, [symbol], self._run_logger
            )
        else:
            run_dir = staging_dir
