...
class _SymbolRoutingHandler(logging.Handler):
    ...
    def __init__(self, exec_log_dir: Path, symbols: List[str]):
        ...
        self._portfolio_handler: logging.FileHandler | None = None
    ...
    def _get_symbol_from_message(self, message: str) -> str | None:
        ...
        if message.startswith('['):
            ...
            if end_bracket > 1:
                ...
                # Check if it's a known symbol (may contain space for timestamp)
                # Format: "SYMBOL timestamp" or just "SYMBOL"
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            # Get the raw message (without formatter prefix) for symbol extraction
...
class BacktestEngine:
    ...
    def run(self) -> PortfolioResult:
        ...
        staging_dir = self._build_run_dir(backtest_start_local, data_start=None, data_end=None)
        staging_dir.mkdir(parents=True, exist_ok=True)
        symbols = [spec.symbol for spec in self._instruments]
        self._ensure_run_log_file(staging_dir, symbols)
        ...
        if len(self._instruments) == 1:
            ...
            single_result = self._run_single_instrument(staging_dir, backtest_start_local, export_trades=True)
        ...
        if not report.safe:
            ...
            return self._run_sequential_multi(staging_dir, backtest_start_local)
        ...
        return self._run_parallel_multi(staging_dir, backtest_start_local)

    def _run_single_instrument(self, staging_dir: Path, backtest_start_local: datetime, export_trades: bool = True) -> SingleInstrumentResult:
        ...
        Args:
            ...
            export_trades: Whether to export trades to CSV (default True for standalone runs)
        ...
        if not synchronized_base_data:
            ...
            logger.info("Run log: %s", staging_dir / _EXECUTION_LOG_DIR)
        ...
        # Parse data_start and data_end as datetime objects for new format
        ...
        final_run_dir = self._build_run_dir(backtest_start_local, data_start_dt, data_end_dt)
        if final_run_dir != staging_dir:
            run_dir = self._promote_run_dir(staging_dir, final_run_dir)
        else:
            run_dir = staging_dir

        # Export closed trades into the final run dir (only for standalone single-instrument runs)
        if export_trades:
            self._export_trades_csv(run_dir)
        logger.info("Run log: %s", run_dir / _EXECUTION_LOG_DIR)
    ...
    def _run_sequential_multi(self, staging_dir: Path, backtest_start_local: datetime) -> PortfolioResult:
        """Run multiple instruments together in a single event loop. Run multiple instruments sequentially in a unified directory.
        
        Uses the original strategy instance for all instruments to maintain
        state across instruments (e.g., portfolio snapshots).
        All instruments share the same PositionManager and OMS for portfolio-level tracking.
        This is the core multi-instrument execution logic that processes
        all symbols together and returns a PortfolioResult.
        """
        ...
        # Build PortfolioResult for all instruments
        ...
        for symbol in symbols:
            ...
            # Calculate trade statistics for this symbol
            ...
            symbol_results[symbol] = SymbolResult(
                ...
                max_drawdown=max_drawdown,  # Use portfolio max drawdown for now
                sharpe_ratio=None,
            )
    ...
    def _run_parallel_multi(self, staging_dir: Path, backtest_start_local: datetime) -> PortfolioResult:
        """Run multiple instruments in parallel using ProcessPoolExecutor.
        
        Each worker writes to its own symbol-specific log file in the shared
        execution_log/ directory. The parent process aggregates results and
        writes the final closed_trades.csv.
        """
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for spec in self._instruments:
                future = executor.submit(
                    ...
                    str(staging_dir),  # Pass staging_dir path for shared logs
                )
        ...
        # The PositionManager in the parent process should have all trades
        # But workers have their own PositionManager instances...
        # We need to collect trades from all workers
        # For now, the workers write their own trades, but we need to aggregate
        # This is a limitation - we'll need to collect trades from results
        all_trades = []
    ...
    def _promote_run_dir(self, staging_dir: Path, final_run_dir: Path) -> Path:
        ...
        symbols = [spec.symbol for spec in self._instruments]
        self._ensure_run_log_file(final_run_dir, symbols)
    ...
    def _ensure_run_log_file(self, run_dir: Path, symbols: List[str] | None = None) -> None:
        ...
        * If a researcher-installed ``FileHandler`` is present and we
        ...
        for h in root.handlers:
            if getattr(h, self._QUANTREX_RUN_HANDLER, False):
                # Rebind all our handlers to the new directory
                self._rebind_log_handlers(root, exec_log_dir, symbols)
                return
    ...
    def _rebind_log_handlers(self, root: logging.Logger, exec_log_dir: Path, symbols: List[str]) -> None:
        """Rebind existing Quantrex log handlers to a new directory."""
        # Find our custom handler and update its directory
        for h in root.handlers:
            if getattr(h, self._QUANTREX_RUN_HANDLER, False) and isinstance(h, _SymbolRoutingHandler):
                h.rebind(exec_log_dir, symbols)
                return
        
        # Fallback: remove old handlers and create new ones
        handlers_to_remove = [h for h in root.handlers if getattr(h, self._QUANTREX_RUN_HANDLER, False)]
    ...
    def _build_run_dir(
        ...
        Format: output/{StrategyName}/{timestamp}*{start_date}*{end_date}/
        Example: output/TestStrategy/20260920_0135*01Jan26_31Jan26/
        """
...
def _run_single_instrument_worker(
    ...
    engine._ensure_run_log_file(staging_dir, symbols)
    
    return engine._run_single_instrument(staging_dir, backtest_start_local, export_trades=False)
